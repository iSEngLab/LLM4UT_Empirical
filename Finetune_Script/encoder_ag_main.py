...
def generate_and_tokenize_prompt(sample, tokenizer, training_args, dataset_type: str = "train"):
    ...
    # The key should be the same as model forward() function
    # TODO: missing codegeex2
    return {
...
def save_predictions(trainer: "Trainer", tokenizer, predict_results: "PredictionOutput") -> None:
    ...
    predictions = predict_results.predictions.squeeze(1)
    preds = np.where(
        predictions != IGNORE_INDEX, predictions, tokenizer.pad_token_id
    )
