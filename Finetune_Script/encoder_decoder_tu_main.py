...
class ModelArguments:
    ...
    use_codet5p_2bp: bool = field(default=False)
...
def save_predictions(trainer: "Trainer", tokenizer, predict_results: "PredictionOutput") -> None:
    ...
    with open(output_prediction_file, "w", encoding="utf-8") as writer:
        ...
        for label, pred in zip(decoded_labels, decoded_preds):
            ...
            res.append(json.dumps({"xmatch": pred==label, "label": label, "predict": pred}, ensure_ascii=False))
        xmatch=round(np.mean(accs)*100,4)
        print("\nXmatch : " + str(xmatch))
        # writer.write("xmatch: " + str(xmatch) + '\n')
        writer.write("\n".join(res))
...
def main():
    ...
    # codet5p-2b，6b，16b使用的config有些特殊，故进行单独处理
    if model_args.use_codet5p_2bp:
