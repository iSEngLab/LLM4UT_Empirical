...
def get_codebleu(refs, hyp, lang, params='0.25,0.25,0.25,0.25'):
    ...
    # preprocess inputs
    pre_references = [[x.split("\t")[1].strip() for x in open(file, 'r', encoding='utf-8').readlines()] for file in
                      refs]
    hypothesis = [x.split("\t")[1].strip() for x in open(hyp, 'r', encoding='utf-8').readlines()]
...
def calc_codebleu(refs: List[List[str]], hyp: List[str], lang='java', params='0.25,0.25,0.25,0.25'):
    alpha, beta, gamma, theta = [float(x) for x in params.split(',')]

    # preprocess inputs
    pre_references = [[x.strip() for ref in refs for x in ref]]
